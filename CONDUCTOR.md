...
- **Tech Stack**: Python 3.11+, PyTorch 2.4.0, CUDA 12.4, FLUX.1-Fill-dev diffusion model, LoRA fine-tuning
- **Main File**: gradio_demo.py (252 lines) - Web interface and pipeline execution
- **Core Mechanic**: Virtual try-on generation using diffusion models with LoRA weights for garment adaptation
- **Key Integration**: Hugging Face (models/weights), Gradio web interface, Safetensors (model storage)
- **Platform Support**: Linux with NVIDIA GPU (28GB VRAM min), RunPod container deployment
- **DO NOT**: Modify model checkpoints directly, avoid VRAM exhaustion, don't run inference without GPU
...
## Quick Reference
**Main Entry Point**: `gradio_demo.py:1-252` - Gradio web interface for virtual try-on
**Core Pipeline**: `omnitry/pipelines/pipeline_flux_fill.py` - FLUX diffusion pipeline implementation
**Model Core**: `omnitry/models/transformer_flux.py` - Transformer model with LoRA adapters
**Configuration**: `configs/omnitry_v1_unified.yaml` - Model hyperparameters and paths
**Demo Interface**: `gradio_demo.py:144-251` - Gradio.Blocks UI definition
**Pipeline Init**: `gradio_demo.py:25-27` - Model and pipeline loading
**Generate Function**: `gradio_demo.py:91-141` - Main try-on generation logic
**Benchmark Eval**: `omnitry_bench/vtryon_metric.py` - Performance evaluation
**Preprocess Ground**: `data_preprocess/infer_ground_objects.py` - Object segmentation
**Preprocess List**: `data_preprocess/infer_list_objects.py` - Object detection
**Preprocess Remove**: `data_preprocess/infer_remove_objects.py` - Object removal

## Current State
- [x] Virtual try-on AI system implementation
- [x] FLUX.1-Fill-dev integration
- [ ] Enhanced benchmarking suite
- [ ] Multi-language interface
- [ ] Web deployment optimization

## Development Workflow
1. **Checkpoint Download**: Get FLUX.1-Fill-dev and LoRA weights via Hugging Face
2. **Environment Setup**: Create conda environment with requirements.txt
3. **Model Configuration**: Update configs/omnitry_v1_unified.yaml for lora_path
4. **Basic Testing**: Run python gradio_demo.py for interface validation
5. **Performance Tuning**: Test with nvidia-smi for VRAM usage, optimize batch sizes
6. **Benchmark Evaluation**: Use omnitry_bench/ scripts for quality assessment

## Task Templates
### 1. Model Weight Testing
1. Download latest LoRA weights to checkpoints/ (see suggested_commands.md)
2. Update configs/omnitry_v1_unified.yaml:1-10 to set new lora_path
3. Test loading: python gradio_demo.py:25-27
4. Generate test images: gradio_demo.py:91-141
5. Evaluate quality: omnitry_bench/vtryon_metric.py
6. Update benchmark results in JOURNAL.md

### 2. Performance Optimization
1. Monitor baseline: run nvidia-smi during gradio_demo.py execution
2. Adjust VRAM settings: configs/omnitry_v1_unified.yaml:1-20
3. Test CPU offload: gradio_demo.py:29-30
4. Run benchmarks: omnitry_bench/vtryon_metric.py
5. Document performance metrics in JOURNAL.md
6. Update ARCHITECTURE.md with improvements

### 3. Interface Enhancement
1. Review current UI: gradio_demo.py:144-251
2. Add new features: gradio_demo.py:150-155 for components
3. Test user flows: gradio_demo.py:210-230 examples
4. Add input validation: gradio_demo.py:90-110
5. Update CONFIG.md with new options
6. Document changes in JOURNAL.md

### 4. Configuration Management
1. Review current config: configs/omnitry_v1_unified.yaml
2. Test parameter variations: gradio_demo.py:25-30 loading
3. Benchmark performance: omnitry_bench/ scripts
4. Update documentation: CONFIG.md
5. Version control changes: git add configs/
6. Journal configuration decisions

## Anti-Patterns (Avoid These)
❌ **Don't modify model checkpoints directly** - Use safetensors format for consistency
❌ **Don't run without GPU** - Requires minimum 28GB VRAM for inference
❌ **Don't forget CUDA_VISIBLE_DEVICES** - Specify GPU device explicitly
❌ **Don't mix float32/float16 weights** - Maintain consistent dtype throughout
❌ **Don't bypass environment.yml** - Use proper conda environment for reproducibility
❌ **Don't commit large models** - Keep checkpoints/ directory gitignored
