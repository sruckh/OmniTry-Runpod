...
def load_model(config_name):
    ...
    global current_model, pipeline, transformer
    
    if current_model == config_name and pipeline is not None:
        return f"Model '{config_name}' is already loaded."
    
    try:
        # Load configuration
        config_path = model_configs[config_name]
        ...
        current_model = config_name
        
        return f"Successfully loaded '{config_name}' model!"
        
    except Exception as e:
        return f"Error loading model '{config_name}': {str(e)}"


def get_current_object_classes():
    """Get object classes for the currently loaded model"""
    if current_model is None:
        return []
    
    config_path = model_configs[current_model]
    args = OmegaConf.load(config_path)
    return list(args.object_map.keys())
...
def generate(person_image, object_image, object_class, steps=20, guidance_scale=30, seed=-1, progress=gr.Progress(track_tqdm=True)):
    ...
    if not person_image or not object_image or not object_class:
        return "Please provide person image, object image, and object class."
...
def on_model_change(model_name):
    ...
    if model_name:
        ...
        # Update object class choices
        object_classes = get_current_object_classes()
        return status, gr.Dropdown(choices=object_classes, value=object_classes[0] if object_classes else None)
    return "No model selected.", gr.Dropdown(choices=[], value=None)


if __name__ == '__main__':
    # Load default model on startup
    default_model = "Unified Model"
    load_model(default_model)

    with gr.Blocks(title="OmniTry - Virtual Try-On") as demo:
        gr.Markdown('# Demo of OmniTry - Virtual Try-On with Model Selection')
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                model_status = gr.Textbox(
                    label="Model Status",
                    value=f"'{default_model}' loaded successfully!",
                    interactive=False,
                    lines=2
                )
        ...
        with gr.Row():
            ...
            with gr.Column():
                object_image = gr.Image(type="pil", label="Object Image", height=600)
                object_class = gr.Dropdown(
                    label='Object Class', 
                    choices=get_current_object_classes(),
                    value=get_current_object_classes()[0] if get_current_object_classes() else None
                )
    ...
    # Launch with more accessible settings
    demo.launch(
        server_name="0.0.0.0",
        server_port=7860,
        share=False,
